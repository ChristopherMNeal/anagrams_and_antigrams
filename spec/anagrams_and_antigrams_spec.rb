...
describe (Text) do
  ...
  describe ("#anagram_checker") do
    it("returns 'neither an anagram nor an antigram! the two inputs have letters {letters in common} in common' if the text is not an anagram") do
      expect(text.anagram_checker("tab")).to(eq("neither an anagram nor an antigram! the two inputs have 2 letters: 'a, t' in common"))
    end
    ...
    it("returns 'neither an anagram nor an antigram! the two inputs have letters {letters in common} in common' if the text is not an anagram") do
      expect(sentence.anagram_checker("Twelves, plusses (+) Ones?")).to(eq("neither an anagram nor an antigram! the two inputs have 10 letters: 'e, l, n, o, p, s, t, u, v, w' in common"))
    end
  end
